...
if(_char != " " && _char != "　"){
	...
	INST.order = _order;
	INST.typer = id;
	INST.fadeout = _fadeout
	INST.fadeout_x = _fadeout_x
	INST.fadeout_y = _fadeout_y
	INST.fadeout_dur = _fadeout_dur
	INST.fadeout_rand = _fadeout_rand
	...
	if _fadein {
		var xx,yy
		INST.alpha = 0
		Anim_Create(INST,"alpha",1,1,0,_alpha,_fadein_dur)
		if abs(_fadein_x) > 0 || abs(_fadein_y) > 0 {
			if _fadein_rand {
				xx = random_range(-_fadein_x,_fadein_x)
				yy = random_range(-_fadein_y,_fadein_y)
			}
			else {
				xx = _fadein_x
				yy = _fadein_y
			...
			INST.x+=xx
			INST.y+=yy
			Anim_Create(INST,"x",2,1,INST.x,-xx,_fadein_dur)
			Anim_Create(INST,"y",2,1,INST.y,-yy,_fadein_dur)
		}
	}
